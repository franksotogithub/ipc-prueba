import {
  ...
  export class MatTableResponsiveDirective implements OnInit, AfterViewInit, OnDestroy {
    ...
    iniciando(){
        ...
        if(!this.table){
            window.setTimeout(this.iniciando,500);
            return;
        }
    ...
    ngAfterViewInit() {
        this.generando();
        /*HTMLTableRowElement[Symbol.iterator] = function* () {
    ...
    generando(){
        ...
        let composeBox=this.thead;

        if(!composeBox) {
            window.setTimeout(this.generando,500);
            return;
        }
